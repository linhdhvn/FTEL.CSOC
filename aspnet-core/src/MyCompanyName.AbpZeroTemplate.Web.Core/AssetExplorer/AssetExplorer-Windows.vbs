...
Set objDateTimeService = CreateObject("WbemScripting.SWbemDateTime")
...
For Each iterResult in queryResult
    ...
    csdata = addCategoryData(csdata, "Domain", iterResult.WorkGroup)
...
'=====================
On Error Resume Next
    Set queryResult = objCIMVService.ExecQuery ("SELECT * FROM Win32_BaseBoard")
    dataText = "<BaseBoard "
    For Each iterResult in queryResult
        bbdata = addCategoryData(bbdata, "Manufacturer", iterResult.Manufacturer)
        bbdata = addCategoryData(bbdata, "Model", iterResult.Product)
        serialNumber = iterResult.SerialNumber
        bbdata = addCategoryData(bbdata, "SerialNumber", iterResult.SerialNumber)
    Next

    Set queryResult = objCIMVService.ExecQuery ("SELECT * FROM Win32_ComputerSystemProduct")
    For Each iterResult in queryResult
        bbdata = addCategoryData(bbdata, "UUID", iterResult.UUID)
    Next

    Set queryResult = objCIMVService.ExecQuery ("SELECT * FROM Win32_BIOS")
    For Each iterResult in queryResult
        bbdata = addCategoryData(bbdata, "BIOSVersion", iterResult.Version)
    Next

    dataText = dataText & bbdata & " />" & newLineConst
...
Dim procIdList()
On Error Resume Next
    Set queryResult = objCIMVService.ExecQuery ("SELECT * FROM Win32_Processor")
    dataText = "<Processors>" & newLineConst
    For Each iterResult in queryResult
        ...
        dataText = addCategoryData(dataText, "ProcessorId", iterResult.ProcessorId)
    ...
    Next
    outputText = outputText & dataText & "</Processors>" & newLineConst
Err.clear

'Physical Memory Info
'=================
On Error Resume Next
    Set queryResult = objCIMVService.ExecQuery ("SELECT * FROM Win32_PhysicalMemory")
    dataText = "<PhysicalMemories>" & newLineConst
    For Each iterResult in queryResult
        ...
        dataText = addCategoryData(dataText, "dev", iterResult.InstallDate)

        dataText = dataText & " />" & newLineConst
    Next
    dataText = dataText & "</PhysicalMemories>" & newLineConst
    outputText = outputText & dataText
Err.clear
...
Dim sMultiStrings()
On Error Resume Next
    Set queryResult = objCIMVService.ExecQuery ("SELECT * FROM Win32_DesktopMonitor where PNPDeviceID != NULL")
    dataText = "<Monitors>" & newLineConst
    ...
    Set queryResult = objWMIService.ExecQuery ("SELECT * FROM WmiMonitorID")
	For Each iterResult in queryResult
		...
		End if
	Next
	dataText = dataText & "</Monitors>" & newLineConst
    outputText = outputText & dataText
Err.clear
...
'=============
On Error Resume Next
    Set queryResult = objCIMVService.ExecQuery ("SELECT * FROM Win32_NetworkAdapterConfiguration where IPEnabled = True")
    dataText = "<NetworkAdapters>" & newLineConst
    For Each iterResult in queryResult
        ...
        dataText = addCategoryData(dataText, "PhysicalAddress", iterResult.MACAddress)
        dataText = addCategoryData(dataText, "UseDHCP", iterResult.DHCPEnabled)
        ...
        dataText = dataText & " />" & newLineConst
    Next
    dataText = dataText & "</NetworkAdapters>" & newLineConst
    outputText = outputText & dataText
Err.clear
...
On Error Resume Next
    ...
    Set queryResult = objCIMVService.ExecQuery ("SELECT * FROM Win32_OperatingSystem")
    dataText = "<OperatingSystem "
    For Each iterResult in queryResult
        osdata = addCategoryData(osdata, "Name", iterResult.Caption)
        osdata = addCategoryData(osdata, "BuildNumber", iterResult.Version)
...
Function getInstallDate(installDate)
	objDateTimeService.Value = installDate
	getInstallDate = Year(objDateTimeService.GetVarDate)&"/"&Month(objDateTimeService.GetVarDate)&"/"&Day(objDateTimeService.GetVarDate)
End Function

Function getLastBootUpTime(lastBootUpTime)
	objDateTimeService.Value = lastBootUpTime
	getLastBootUpTime = Year(objDateTimeService.GetVarDate)&"/"&Month(objDateTimeService.GetVarDate)&"/"&Day(objDateTimeService.GetVarDate)&" "&Hour(objDateTimeService.GetVarDate)&":"&Minute(objDateTimeService.GetVarDate)&":"&Second(objDateTimeService.GetVarDate)
End Function
