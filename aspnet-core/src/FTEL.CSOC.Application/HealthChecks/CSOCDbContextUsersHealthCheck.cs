...
namespace FTEL.CSOC.HealthChecks
{
    public class CSOCDbContextUsersHealthCheck : IHealthCheck
    {
        ...
        public async Task<HealthCheckResult> CheckHealthAsync(HealthCheckContext context, CancellationToken cancellationToken = new CancellationToken())
        {
                    ...
                    using (_unitOfWorkManager.Current.SetTenantId(null))
                    {
                        ...
                        if (!await dbContext.Database.CanConnectAsync(cancellationToken))
                        {
                            return HealthCheckResult.Unhealthy(
                                "CSOCDbContext could not connect to database"
                            );
                        }
                        ...
                        if (user)
                        {
                            return HealthCheckResult.Healthy("CSOCDbContext connected to database and checked whether user added");
                        }

                        return HealthCheckResult.Unhealthy("CSOCDbContext connected to database but there is no user.");
            ...
            {
                return HealthCheckResult.Unhealthy("CSOCDbContext could not connect to database.", e);
            }
        }
    }
}
